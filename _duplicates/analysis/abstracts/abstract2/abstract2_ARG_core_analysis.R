...
# Original had: setwd("...") which is invalid R syntax (formula) and typo-prone.
...
# ====== 1) Paths & parameters ======
# Optional geospatial/admin mapping inputs (kept generic for portfolio)
admin_shapefile_dir <- "external_metadata/admin_boundaries"  # expects adm1.shp..adm4.shp (or adjust in code)
external_metadata_dir <- "external_metadata"
...
masterlist_file <- "metadata/sample_masterlist.tsv"  # optional: drives sample exclusions/covariates when present
exclude_samples_file <- "metadata/exclude_samples.tsv"  # optional: 1 column of sample IDs to drop
...
if(fs::file_exists(masterlist_file)){
  ...
  if(!is.na(sample_code_col)){
    ...
    # Fallback: if no sample column detected, try the first column ('x') if it looks like string sample IDs
    if('x' %in% names(master) && all(str_detect(as.character(master$x[!is.na(master$x)]), '^[A-Za-z0-9][A-Za-z0-9._-]+$'))) master <- master %>% rename(sample = x)
  }
  ...
  if(fs::file_exists(masterlist_file)){
    ...
    if(!is.na(lat_col) && !is.na(lon_col) && 'sample_base' %in% names(mlc)){
      ...
      if(nrow(pts) > 0){
        ...
        utm_crs <- 3857
        sf_pts_utm <- try(sf::st_transform(sf_pts, utm_crs), silent=TRUE)
        ...
        read_layer <- function(lv){
          f <- file.path(admin_shapefile_dir, sprintf('adm%d.shp', lv))
        ...
        for(lv in 1:4){
          ...
          tryCatch({
            lyr_path <- file.path(admin_shapefile_dir, sprintf('adm%d.shp', lv))
...
try({
  if(exists('admin_mapping') && !is.null(admin_mapping) && 'adm4_unit' %in% names(admin_mapping)){
    ...
    adm4_lookup <- try({
      sf::st_read(file.path(admin_shapefile_dir, 'adm4.shp'), quiet=TRUE) %>%
        sf::st_drop_geometry() %>%
    ...
    admin_map_codes <- if(!is.null(adm4_lookup)) {
      ...
      # If many codes do not map, keep NA values and reconcile externally.
      }
      joined
    } else admin_mapping
    # External datasets
    path_meta <- external_metadata_dir
    ...
    f_google <- file.path(path_meta,'open_buildings_google.csv')
    f_tm     <- file.path(path_meta,'open_buildings_tm.csv')
    f_water  <- file.path(path_meta,'osm_poi_water.csv')
    f_san    <- file.path(path_meta,'osm_poi_sanitation.csv')
    f_census <- file.path(path_meta,'census_population_adm4.xlsx')
    ...
    if(!is.null(census_tbl)){
      ...
      if(!is.na(pcode_col)){
        ...
        # Standardize subdistrict name casing to match admin_map_codes
        census_tbl <- census_tbl %>% mutate(subdistrict_std = tolower(stringr::str_replace_all(subdistrict, '^subdistrict\\s+',''))) %>%
          select(adm4_pcode, census__subdistrict = subdistrict_std, census__pop2020 = any_of('x2020_census_popn'))
