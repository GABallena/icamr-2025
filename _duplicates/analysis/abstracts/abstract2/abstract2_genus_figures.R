...
infile <- Sys.getenv("PATHOGEN_GENUS_TABLE", unset = "results/tables/pathogen_genus_abundance.tsv")  # change if needed
# Optional curated pathogen list (single column "genus"); set to NULL to skip
pathogen_list_file <- Sys.getenv("PATHOGEN_LIST", unset = "")
# If PATHOGEN_LIST is empty, no curated list is used.
...
if (pathogen_list_file == "") pathogen_list_file <- NULL
# -------------------------------------------------------------
